...
> Mixing PyTorch + Python Notebooks + Rust
...
Out of curiousity, I set up a hybrid Rust + PyTorch development environment and
re-implemented nanoGPT in Rust using the [PyTorch Rust
bindings](https://github.com/LaurentMazare/tch-rs). This post will walk through
how I did this so you can add Rust to your LLM development as well!

# Starting point: `nanogpt.py`

First, let's get our initial Python environment set up. Starting from a fresh [`conda`](https://conda.io) environment:
...
Python Notebook cells. The first cell in `nanogpt.ipynb` sould plot the count of each
...
run on GPUs. But bata processing code is generally bespoke Python, which can be
...
Whenever we make a change to the Rust code, just run `maturin develop` to build
...
We can automatically re-run `maturin develop` using [`cargo-watch`](https://github.com/watchexec/cargo-watch):
...
Now we can compare the two implementations in our Notebook:
...
impl Hasher for CharHasher {
    fn write(&mut self, bytes: &[u8]) {
        //  `char` in Rust is fixed-width with 4 bytes.
        if bytes.len() != 4 {
            return;
        }
        self.c =
            char::from_u32(u32::from_le_bytes(bytes.try_into().unwrap_or([0; 4]))).unwrap_or('a');
    }
...
> Note that this hash function only works for `char`s and relies on the
> implementation details of `std::collections::HashMap`. It could be made more
> generic with a little extra logic like tracking how many bytes have been
> written so far.
...
avoiding a `HashSet` altogether. I personally liked that this solution kept all
of the ergonomics of `HashSet`.
