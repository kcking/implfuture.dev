...
into either a `.tsx` component or directly used in a `.mdx` file.
...
> The `pub` and `#[wasm_bindgen]` will make this function callable from anything
...
`next build`. My whole build currently takes just over 2 minutes, but 90% of
that is installing these build depencencies. I hope Vercel considers adding rust + wasm-pack to a pre-baked build image!

Thanks for reading! I hope this post helped smooth out a few of the rough edges
with getting rust integrated into your own blog. Feel free to clone, fork, or pull request this site on [GitHub](https://github.com/kcking/implfuture.dev)!
